...
class StockDataApp:
    ...
    class Technical_analysis(): # Adding a nested class so that later the app can be upgraded more easily
        ...
        def MACD_plot(self):
            ...
            # Calculate MACD
            # Standard MACD with fast = 12 and slow = 26
            self.closedf = pd.DataFrame(self.ta_data["4. close"])
            self.closedf = self.closedf.rename(columns={"4. close": "close"})
            self.macddf = self.closedf.ta.macd(fast=12, slow=26, signal=9, min_periods=None, append=True) 

            
            # Apply dark style before plotting
            dark_style = {
                "axes.facecolor": "#222222",
                "axes.edgecolor": "white",
                "axes.labelcolor": "white",
            ...
            # Finalize the plot
            plt.legend(loc='best')
            plt.title(f'{self.symbol} MACD and Signal Line with Histogram')
            plt.xlabel('Date')
            plt.ylabel('Value')
